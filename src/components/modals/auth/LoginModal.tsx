...
const LoginModal = () => {
  ...
  const [shouldFetchStore, setShouldFetchStore] = useState(false);
  const { data: stores } = useGetAllStoreQuery(undefined, {
    skip: !shouldFetchStore,
  });
  const noEntry =
  ...
  }, [currentUser, setShouldFetchStore]);
  // console.log("stores", stores)
  // navigate to the store dashboard or go to onboarding if no stores
  useEffect(() => {
    if (stores && stores?.length > 0) {
      // Navigate to the first Store's dashboard
    ...
    <div className="h-[100vh]  bg-[#16161639] inset-0 backdrop-blur-sm w-full fixed top-0 left-0 z-[5000] flex items-end lg:items-center lg:justify-center">
      ...
      >
        <div
          ...
          className="absolute top-2 md:top-4 right-4 text-[#000] cursor-pointer w-10 lg:w-12 h-10 lg:h-12 flex items-center hover:bg-[#7777772a] rounded-full justify-center z-[20] text-xl"
        >
          <RxCross2 />
        </div>
        <div className="w-full h-full grid lg:grid-cols-2">
          <div className="w-full flex flex-col justify-center gap-4 py-16 md:py-12 px-10">
            <div className="w-full flex flex-col gap-1">
              <h3 className="text-2xl capitalize md:text-3xl font-selleasy_regular">
                Sign in & take control of your stores!
              </h3>
              <span className="block text-base md:text-sm font-work_font max-w-[250px] pt-1">
                Your business, your stores—just one login away!
              </span>
            </div>
            <form
            ...
            >
              ...
              <div className="w-full mt-4 flex items-center justify-center flex-col gap-3">
                <button
                  ...
                  type="submit"
                  disabled={noEntry}
                  className="flex btn items-center justify-center w-full cursor-pointer rounded-md font-selleasy_regular"
                >
                  {isLoading ? (
                ...
                <div className="w-full flex items-center justify-start gap-2">
                  <span className="text-sm font-normal text-dark">
                    <span className="">Not yet a Seller?</span>{" "}
                    <span
